...
# username = file_run.py, name == username, look in user.py
def worked():
    login_window.destroy()
    root.destroy()
    subprocess.call(["python", username + ".py"])
...
def submit_form():
  global entry1, entry2, username, login_window, username
...
def submit_sign_form():
    ...
    if user1 and passw2:
      ...
      with open("file_run.py", "r") as thefile:
        main = thefile.read()
        thefile.close()
      with open(username1 + ".py", "a") as file2:
        file2.write(main)
        file2.close()
