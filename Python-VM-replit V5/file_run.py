...
def openit(file_name, file_content, Filename):
   print(file_name)
   print(file_content)
   print(Filename)
...
def openfile(file_cont, Filename):
    ...
    b = tk.Button(file_open, text="Open File", command=lambda: openit(file_name, file_content, Filename))
    b.pack()
...
def notepad():
    ...
    file_entry = tk.Text(notepad, width=60, height=35)
    file_entry.pack()
...
toolsmenu.add_command(label="shell", command=shell)
